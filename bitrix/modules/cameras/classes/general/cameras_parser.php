...
class CCamerasParser
{
	public $arNocameras = array();
	public $arCode = array();
	public $arLink = array();
	...
	function Parse($text, $type = 'text', $arFile = array(), $arParams = array())
	{
		...
		// cut nocameras
		$text = preg_replace_callback('/(<nocameras>(.*)<\/nocameras>)/isU'.BX_UTF_PCRE_MODIFIER, array(&$this, '_noCamerasCallback'), $text);

		#if($this->textType == "html")
		#	$text = CCamerasUtils::htmlspecialchars_decode($text);
		...
		// Paste nocameras
		if (!empty($this->arNocameras))
			$text = preg_replace_callback('/(##NOCAMERAS(\d+)##)/isU'.BX_UTF_PCRE_MODIFIER, array(&$this, '_noCamerasReturnCallback'), $text);


		$text = preg_replace_callback('/(##NOCAMERAS(\d+)##)/isU'.BX_UTF_PCRE_MODIFIER, array(&$this, '_noCamerasReturn2Callback'), $text);
	...
	function _processFileCallback($matches)
	{
		...
		$sFile = $sFileName = $sPath = CCamerasUtils::htmlspecialcharsback(trim($matches[3]));
		$bOur = false;
			...
			if ($bOur)
			{
				...
				else
				{
					$sReturn  = CFile::ShowImage($sFile,
						COption::GetOptionString('cameras', 'image_max_width', 600),
						COption::GetOptionString('cameras', 'image_max_height', 600),
	...
	function processInternalLink($text)
	{
		...
		if (!empty($this->arLink))
		{
			...
			while($obElement = $rsElement->GetNextElement())
			{
				...
				$this->arLinkExists[] = strtolower(CCamerasUtils::htmlspecialcharsback($arFields['NAME'], true));
			}
		}
	...
	function _processInternalLinkPrepareCallback($matches)
	{
		...
		{
			if (CCamerasUtils::IsCategoryPage($sName, $sCatName))
				return '##Category##';
		}

		$sTitle = strip_tags($sTitle);
		$i = count($this->arLink);
		$this->arLink[] = CCamerasUtils::htmlspecialcharsback($matches[1], true);
	...
	function _processInternalLinkCallback($matches)
	{
		...
		if (in_array(strtolower($this->arLink[$matches[2]]), $this->arLinkExists))
		{
			$sURL = CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_POST'],
				array(
					'cameras_name' => rawurlencode($this->arLink[$matches[2]]),
		...
		else
		{
			...
			$sReturn = 'href="'.$sURL.'" class="cameras_red"';
		}
	...
	function _codeCallback($matches)
	{
		...
		if($this->textType == "html")
			$codeText = CCamerasUtils::htmlspecialchars_decode($codeText);
	...
	function _noCamerasCallback($matches)
	{
		$i = count($this->arNocameras);
		$this->arNocameras[] = $matches[2];
	...
	function _noCamerasReturnCallback($matches)
	{
		return $this->arNocameras[$matches[2]];
	}

	function _noCamerasReturn2Callback($matches)
	{
		return '<nocameras>'.htmlspecialcharsbx($this->arNocameras[$matches[2]]).'</nocameras>';

	}

	function parseBeforeSave($text, &$arCat = array(), $nameTemplate = "")
	{
		$userLogin = CCamerasUtils::GetUserLogin(array(), $nameTemplate);
		...
		// Category
		$matches = array();
		$textWithoutNocameras = preg_replace('/(<nocameras>(.*)<\/nocameras>)/isU'.BX_UTF_PCRE_MODIFIER, '', $text);
		if (preg_match_all('/\[\[(Category|'.GetMessage('CATEGORY_NAME').'):(.+)\]\]/iU'.BX_UTF_PCRE_MODIFIER, $textWithoutNocameras, $matches))
