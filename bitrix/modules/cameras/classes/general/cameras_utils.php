...
class CCamerasUtils
{
		...
		if (self::IsWriteable() && (!in_array('history', $arPage) && !in_array('history_diff', $arPage)))
		{
			$arLinks['add'] = array(
				...
				'LINK' => CHTTP::urlAddParams(
					CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_POST_EDIT'],
						array(
							'cameras_name' => GetMessage('CAMERAS_NEW_PAGE_TITLE'),
							'group_id' => CCamerasSocnet::$iSocNetId
						)
					),
			...
			if (!in_array('add', $arPage))
			{
				...
				$url = $APPLICATION->GetPopupLink(
					array(
						'URL' => CHTTP::urlAddParams(
							CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_POST_EDIT'],
								array(
									'cameras_name' => rawurlencode($arParams['ELEMENT_NAME']),
									'group_id' => CCamerasSocnet::$iSocNetId
								)
							),
				...
				$arLinks['rename'] = array(
					'NAME' => GetMessage('CAMERAS_PAGE_RENAME'),
					'TITLE' => GetMessage('CAMERAS_PAGE_RENAME_TITLE'),
					...
					$url = $APPLICATION->GetPopupLink(
						array(
							'URL' => CHTTP::urlAddParams(
								CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_POST_EDIT'],
									array(
										'cameras_name' => $arParams['ELEMENT_ID'],
										'group_id' => CCamerasSocnet::$iSocNetId
									)
								),
	...
	{
		if (COption::GetOptionString('cameras', 'allow_html', 'Y') == 'N')
			return false;
	...
	static function CheckAccess($access = 'view')
	{
		...
		{
			$arSonetGroup = CSocNetGroup::GetByID(CCamerasSocnet::$iSocNetId);
			if ($arSonetGroup && CSocNetUser::IsCurrentUserModuleAdmin($arSonetGroup['SITE_ID']))
				return true;

			if (!CSocNetFeaturesPerms::CanPerformOperation($USER->GetID(), SONET_ENTITY_GROUP, CCamerasSocnet::$iSocNetId, 'cameras', $access))
				return false;
		...
		else
		{
			...
			$camerasModulePermission = $APPLICATION->GetGroupRight('cameras');
			$iblockPermission = CIBlock::GetPermission($arParams['IBLOCK_ID']);
			return $camerasModulePermission >= $letter && $iblockPermission >= $letterI;
		}
	}
	...
	static function OnBeforeIndex($arFields)
	{
		...
		$arFields['BODY'] = $CCamerasParser->parseForSearch($arFields['BODY']);
	...
	* @param str $urlRewriterPath - wich path leads to curent module (complex component) ( for example: "/services/cameras.php" )
	...
	*/
	static function SetCommentPath($forumID, $rightPath, $urlRewriterPath)
	{
		...
		$arActualCommentsPath = CCamerasUtils::GetCommentPath($forumID);
	...
	*/
	...
	static function isVirtualCategoryExist($categoryName)
	{
		...
		if($categoryName == GetMessage("CAMERAS_CATEGORY_NOCAT") || $categoryName == GetMessage("CAMERAS_CATEGORY_ALL"))
		{
			$result = true;
		}
		else
		{
			$categories = new CCamerasCategories;
			$rsHandlers = GetModuleEvents("cameras", "OnCategoryListCreate");
