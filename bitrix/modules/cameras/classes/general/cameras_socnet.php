...
class CCamerasSocnet
{
	...
	static function Init($SOCNET_GROUP_ID, $IBLOCK_ID)
	{
		...
		self::$bInit = CSocNetFeatures::IsActiveFeature(SONET_ENTITY_GROUP, self::$iSocNetId, 'cameras');
		return self::$bInit;
	}
	...
	static function IsEnabledSocnet()
	{
		...
		{
			if($arEvent['TO_MODULE_ID'] == 'cameras'
				&& $arEvent['TO_CLASS'] == 'CCamerasSocnet')
			{
				$bActive = true;
				break;
			}
	...
	static function EnableSocnet($bActive = false)
	{
		if($bActive)
		{
			if(!self::IsEnabledSocnet())
			{
				RegisterModuleDependences('socialnetwork', 'OnFillSocNetFeaturesList', 'cameras', 'CCamerasSocnet', 'OnFillSocNetFeaturesList');
				RegisterModuleDependences('socialnetwork', 'OnFillSocNetMenu', 'cameras', 'CCamerasSocnet', 'OnFillSocNetMenu');
				RegisterModuleDependences('socialnetwork', 'OnParseSocNetComponentPath', 'cameras', 'CCamerasSocnet', 'OnParseSocNetComponentPath');
				RegisterModuleDependences('socialnetwork', 'OnInitSocNetComponentVariables', 'cameras', 'CCamerasSocnet', 'OnInitSocNetComponentVariables');
			}
		}
		else
		{
			if(self::IsEnabledSocnet())
			{
				UnRegisterModuleDependences('socialnetwork', 'OnFillSocNetFeaturesList', 'cameras', 'CCamerasSocnet', 'OnFillSocNetFeaturesList');
				UnRegisterModuleDependences('socialnetwork', 'OnFillSocNetMenu', 'cameras', 'CCamerasSocnet', 'OnFillSocNetMenu');
				UnRegisterModuleDependences('socialnetwork', 'OnParseSocNetComponentPath', 'cameras', 'CCamerasSocnet', 'OnParseSocNetComponentPath');
				UnRegisterModuleDependences('socialnetwork', 'OnInitSocNetComponentVariables', 'cameras', 'CCamerasSocnet', 'OnInitSocNetComponentVariables');
			}
		}
	}
	...
	{
		$arSocNetFeaturesSettings['cameras'] = array(
			'allowed' => array(SONET_ENTITY_GROUP),
			'title' => GetMessage('CAMERAS_SOCNET_TAB'),
			'operations' => array(
			...
			'operation_titles' => array(
				'view' => GetMessage('CAMERAS_PERM_READ'),
				'write' => GetMessage('CAMERAS_PERM_WRITE'),
				'delete' => GetMessage('CAMERAS_PERM_DELETE')
			),
			...
			'subscribe_events' => array(
				'cameras' => array(
					'ENTITIES' => array(
						SONET_SUBSCRIBE_ENTITY_GROUP => array(
							'TITLE' => GetMessage('SOCNET_LOG_CAMERAS_GROUP'),
							'TITLE_SETTINGS' => GetMessage('SOCNET_LOG_CAMERAS_GROUP_SETTINGS'),
							'TITLE_SETTINGS_1' => GetMessage('SOCNET_LOG_CAMERAS_GROUP_SETTINGS_1'),
							'TITLE_SETTINGS_2' => GetMessage('SOCNET_LOG_CAMERAS_GROUP_SETTINGS_2')
						),
					),
					'OPERATION' => 'view',
					'CLASS_FORMAT' => 'CCamerasSocnet',
					'METHOD_FORMAT' => 'FormatEvent_Cameras',
					'HAS_CB' => 'Y',
					'FULL_SET' => array("cameras", "cameras_del", "cameras_comment"),
					"COMMENT_EVENT" => array(
						"EVENT_ID" => "cameras_comment",
						"OPERATION" => "view",
						"OPERATION_ADD" => "view",
						"ADD_CALLBACK" => array("CCamerasSocnet", "AddComment_Cameras"),
						"UPDATE_CALLBACK" => array("CSocNetLogTools", "UpdateComment_Forum"),
						"DELETE_CALLBACK" => array("CSocNetLogTools", "DeleteComment_Forum"),
						"CLASS_FORMAT" => "CCamerasSocnet",
						"METHOD_FORMAT" => "FormatComment_Cameras",
				...
				'cameras_del' => array(
					'ENTITIES' => array(
						SONET_SUBSCRIBE_ENTITY_GROUP => array(
							'TITLE' => GetMessage('SOCNET_LOG_CAMERAS_DEL_GROUP')
						)
					),
					'OPERATION' => 'view',
					'CLASS_FORMAT' => 'CCamerasSocnet',
	...
	{
		$arResult['AllowSettings']['cameras'] = true;

		$arResult['CanView']['cameras'] = ((array_key_exists('ActiveFeatures', $arResult) ? array_key_exists('cameras', $arResult['ActiveFeatures']) : true) && CSocNetFeaturesPerms::CanPerformOperation($GLOBALS['USER']->GetID(), $arParams['ENTITY_TYPE'], $arParams['ENTITY_ID'], 'cameras', 'view', CSocNetUser::IsCurrentUserModuleAdmin()));
		$arResult['Title']['cameras'] = (array_key_exists('ActiveFeatures', $arResult) && array_key_exists('cameras', $arResult['ActiveFeatures']) && strlen($arResult['ActiveFeatures']['cameras']) > 0 ? $arResult['ActiveFeatures']['cameras'] : GetMessage('CAMERAS_SOCNET_TAB'));

		if (!array_key_exists('SEF_MODE', $arResult) || $arResult['SEF_MODE'] != 'N')
			$arResult['Urls']['cameras'] = $arResult['Urls']['view'].'cameras/';
		else
		{
			...
			$arResult['Urls']['cameras'] = '?'.$arResult['PAGE_VAR'].'=group_cameras_index&'.$arResult['GROUP_VAR'].'='.$arResult['Group']['ID'];
		}
	}
	...
	static function OnParseSocNetComponentPath(&$arUrlTemplates, &$arCustomPagesPath, $arParams)
	{
		if ($arParams['SEF_MODE'] == 'N')
		{
			$arMyUrlTemplates = array(
				'group_cameras_index' => 'page=group_cameras_index&group_id=#group_id#',
				'group_cameras_categories' => 'page=group_cameras_categories&group_id=#group_id#',
				'group_cameras_search' => 'page=group_cameras_search&group_id=#group_id#',
				'group_cameras_post' => 'page=group_cameras_post&group_id=#group_id#&title=#cameras_name#',
				'group_cameras_post_edit' => 'page=group_cameras_post_edit&group_id=#group_id#&title=#cameras_name#',
				'group_cameras_post_history' => 'page=group_cameras_post_history&group_id=#group_id#&title=#cameras_name#',
				'group_cameras_post_history_diff' => 'page=group_cameras_post_history_diff&group_id=#group_id#&title=#cameras_name#',
				'group_cameras_post_discussion' => 'page=group_cameras_post_discussion&group_id=#group_id#&title=#cameras_name#',
				'group_cameras_post_category' => 'page=group_cameras_post_category&group_id=#group_id#&title=#cameras_name#',
				'group_cameras_post_comment' => 'page=group_cameras_post_comment&#message_id=#message_id#'
			);
		}
		else
		{
			$arMyUrlTemplates = array(
				'group_cameras_index' => 'group/#group_id#/cameras/',
				'group_cameras_categories' => 'group/#group_id#/cameras/categories/',
				'group_cameras_search' => 'group/#group_id#/cameras/search/',
				'group_cameras_post' => 'group/#group_id#/cameras/#cameras_name#/',
				'group_cameras_post_edit' => 'group/#group_id#/cameras/#cameras_name#/edit/',
				'group_cameras_post_history' => 'group/#group_id#/cameras/#cameras_name#/history/',
				'group_cameras_post_history_diff' => 'group/#group_id#/cameras/#cameras_name#/history/diff/',
				'group_cameras_post_discussion' => 'group/#group_id#/cameras/#cameras_name#/discussion/',
				'group_cameras_post_category' => 'group/#group_id#/cameras/#cameras_name#/',
				'group_cameras_post_comment' => 'group/#group_id#/cameras/#cameras_name#/?MID=#message_id##message#message_id#'
			);
		}
		...
		if(!$base_path)
		{
			if(file_exists($_SERVER['DOCUMENT_ROOT'].'/bitrix/php_interface/cameras/'.SITE_ID.'/group_index.php'))
				$base_path = '/bitrix/php_interface/cameras/'.SITE_ID.'/';
			elseif(file_exists($_SERVER['DOCUMENT_ROOT'].'/bitrix/php_interface/cameras/group_index.php'))
				$base_path = '/bitrix/php_interface/cameras/';
			else
				$base_path = '/bitrix/modules/cameras/socnet/';
		}
	...
	{
		$arVariableAliases['cameras_name'] = 'cameras_name';
		$arVariableAliases['title'] = 'title';
		$arVariableAliases['oper'] = 'oper';
		$arVariableAliases['message_id'] = 'message_id';
	}

	static function FormatEvent_Cameras($arFields, $arParams, $bMail = false)
	{
		$GLOBALS['APPLICATION']->SetAdditionalCSS('/bitrix/themes/.default/cameras_sonet_log.css');
		...
		$arResult = array(
			...
			"CACHED_CSS_PATH" => array("/bitrix/themes/.default/cameras_sonet_log.css")
		);
		...
		)
		{
			if ($bMail)
			{
				...
				$arResult['ENTITY']['TYPE_MAIL'] = GetMessage('CAMERAS_SOCNET_LOG_ENTITY_G');
			}
			else
			{
		...
		if (
			...
			$cameras_tmp = '<a href="'.$arFields['URL'].'">'.$arFields['TITLE'].'</a>';
		else
			$cameras_tmp = $arFields['TITLE'];

		if ($arFields['EVENT_ID'] == 'cameras')
		{
			$title_tmp = ($bMail ? GetMessage('CAMERAS_SOCNET_LOG_TITLE_MAIL') : GetMessage('CAMERAS_SOCNET_LOG_TITLE'));
			$title_tmp_24 = GetMessage("CAMERAS_SOCNET_LOG_TITLE_24");
		}
		elseif ($arFields['EVENT_ID'] == 'cameras_del')
		{
			$title_tmp = ($bMail ? GetMessage('CAMERAS_DEL_SOCNET_LOG_TITLE_MAIL') : GetMessage('CAMERAS_DEL_SOCNET_LOG_TITLE'));
			$title_tmp_24 = GetMessage("CAMERAS_DEL_SOCNET_LOG_TITLE_24");
		}

		$title = str_replace(
			array('#TITLE#', '#ENTITY#', '#CREATED_BY#'),
			array($cameras_tmp, $arResult['ENTITY']['FORMATTED'], ($bMail ? $arResult['CREATED_BY']['FORMATTED'] : '')),
			$title_tmp
		);
		...
		else
		{
			...
			{
				$GLOBALS['APPLICATION']->SetAdditionalCSS('/bitrix/components/bitrix/cameras.show/templates/.default/style.css');
				$arResult["CACHED_CSS_PATH"][] = "/bitrix/components/bitrix/cameras.show/templates/.default/style.css";
	...
	static function FormatComment_Cameras($arFields, $arParams, $bMail = false, $arLog = array())
	{
		$arResult = array(
			"EVENT_FORMATTED" => array()
		);
		...
		if (
			...
			$cameras_tmp = '<a href="'.$arLog['URL'].'">'.$arLog['TITLE'].'</a>';
		else
			$cameras_tmp = $arLog['TITLE'];

		$title = str_replace(
			array('#TITLE#', '#ENTITY#', '#CREATED_BY#'),
			array($cameras_tmp, $arResult['ENTITY']['FORMATTED'], ($bMail ? $arResult['CREATED_BY']['FORMATTED'] : '')),
			($bMail ? GetMessage('CAMERAS_SOCNET_LOG_COMMENT_TITLE_MAIL') : GetMessage('CAMERAS_SOCNET_LOG_COMMENT_TITLE'))
		);
	...
	static function AddComment_Cameras($arFields)
	{
					...
					else
					{
						...
						)
						{
							...
							if (!$arUnFollower)
							{
								...
								$arMessageFields["NOTIFY_TAG"] = "CAMERAS|COMMENT|".$arElement['ID'];
								$arMessageFields["NOTIFY_MESSAGE"] = GetMessage("CAMERAS_SONET_FROM_LOG_IM_COMMENT", Array(
									"#title#" => (
										strlen($url) > 0 
								...
								$arMessageFields["NOTIFY_MESSAGE_OUT"] = GetMessage("CAMERAS_SONET_FROM_LOG_IM_COMMENT", Array(
	...
	static function RecalcIBlockID($SocNetGroupID)
	{
		...
		$arCamerasIblockID = array();
		$iblock_id_tmp = COption::GetOptionString("cameras", "socnet_iblock_id", false, "");
		if (intval($iblock_id_tmp) > 0)
			$arCamerasIblockID[] = $iblock_id_tmp;

		$rsSite = CSite::GetList($by="sort", $order="asc", array("ACTIVE"=>"Y"));
		while($arSite = $rsSite->Fetch())
		{
			$iblock_id_tmp = COption::GetOptionString("cameras", "socnet_iblock_id", false, $arSite["LID"]);
			if (intval($iblock_id_tmp) > 0)
				$arCamerasIblockID[] = $iblock_id_tmp;
		}

		if (count($arCamerasIblockID) > 0)
		{
			$rsCamerasSection = CIBlockSection::GetList(
				array("timestamp_x"=>"desc"),
				array(
					"IBLOCK_ID" => array_unique($arCamerasIblockID),
					"SOCNET_GROUP_ID" => $SocNetGroupID
				),
				false,
				array("IBLOCK_ID")
			);
			if ($arCamerasSection = $rsCamerasSection->Fetch())
				return $arCamerasSection["IBLOCK_ID"];
		}
...
*/
		$retText = "<div class='cameras_post_feed'>".$text."</div>";
	...
	function BeforeIndexSocNet($bxSocNetSearch, $arFields)
	{
		...
		if (!$isSonetEnable)
		{
			$isSonetEnable = COption::GetOptionString('cameras', 'socnet_enable');	
		}

		if (!$sonetForumId)
		{
			$sonetForumId = intval(COption::GetOptionString('cameras', 'socnet_forum_id'));
		}
				...
				$arFields = $bxSocNetSearch->BeforeIndexForum(
					...
					$bxSocNetSearch->Url(
						str_replace(
							...
							$bxSocNetSearch->_params["PATH_TO_GROUP_CAMERAS_POST_COMMENT"]
						),
						array(
							"MID" => "#message_id#"
						), 
