...
use \Bitrix\Cameras\Integration;
...
final class LivefeedIndexLog extends Stepper
{
	...
	{
		if (!(
			Loader::includeModule("cameras")
			&& Loader::includeModule("socialnetwork")
			&& Option::get('cameras', 'needLivefeedIndexLog', 'Y') == 'Y'
		))
		{
			return false;
		}
		...
		$params = Option::get("cameras", "livefeedindexlog", "");
		...
		if ($params["count"] > 0)
		{
			$result["title"] = Loc::getMessage("FUPD_LF_CAMERAS_LOG_EVENT_INDEX_TITLE");
			$result["progress"] = 1;
			$result["steps"] = "";
			...
			if ($found)
			{
				Option::set("cameras", "livefeedindexlog", serialize($params));
				$return = true;
			}
			...
			if ($found === false)
			{
				Option::delete("cameras", array("name" => "livefeedindexlog"));
				Option::set('cameras', 'needLivefeedIndexLog', 'N');
			}
		}
		return $return;
	}
